...
def build_json(blobstore_name, blob_id, soft_quota_enabled, soft_quota_type, soft_quota_limit):
    data = {}
    data['path'] = blob_id
    data['name'] = blobstore_name
    if soft_quota_enabled == 'Enabled':
        quota = {}
        quota['type'] = soft_quota_type
        quota['limit'] = soft_quota_limit * 1000 * 1000
...
def main():
    ...
    argument_spec.update(
        ...
        blob_info=dict(type='dict', options=dict(
            ...
            soft_quota_enabled=dict(type='str', default='Disabled', choices=['Enabled', 'Disabled', 'Maintain']),
...
#    body = module.params['body']
    ...
    if module.params['blob_info'] is None:
        blobstore_name = None
        blob_id = None
        soft_quota_enabled = None
        soft_quota_type = None
        soft_quota_limit = None
    else:
        fail, msg, blobstore_name, blob_id, soft_quota_enabled, soft_quota_type, soft_quota_limit = \
            parse_blob_info(module.params['blob_info'])
        if fail:
            uresp['msg'] = msg
    ...
    url = build_url(base_url, endpoint, blobstore_name, blob_id, method)
    body = None
    if method == 'POST':
        body = build_json(blobstore_name, blob_id, soft_quota_enabled, soft_quota_type, soft_quota_limit)
    if body_format == 'json':
        # Encode the body unless its a string, then assume it is pre-formatted JSON
        if not isinstance(body, string_types):
            body = json.dumps(body)
            uresp['msg'] = 'Hits this shit'
