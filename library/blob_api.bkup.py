...
options:
  ...
  blob_id:
    description:
      - Nexus ID of blob, it's the path for the blob.
    type: str
  blobstore:
    description:
      - Name of the blobstore in Nexus
    type: str
  return_content:
    description:
...
def build_URL(baseurl, endpoint, blobstore, blob_id):
...
def main():
    ...
    argument_spec.update(
        ...
        blobstore=dict(type='str'),
        blob_id=dict(type='str')
    )
    ...
    blobstore_name = module.params['blobstore']
    blobID = module.params['blob_id']
    socket_timeout = 30

    dict_headers = module.params['headers']

    url = build_URL(base_url, endpoint, blobstore_name, blobID)
    if body_format == 'json':
    ...
    if method == "DELETE" and resp['status'] == 404:
        uresp['msg'] = 'Unable to locate asset by ID'
        uresp['changed'] = False
